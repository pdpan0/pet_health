    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `Pet` (`id` INTEGER, `name` TEXT NOT NULL, `race` INTEGER NOT NULL, PRIMARY KEY (`id`))');
...
class _$PetDAO extends PetDAO {
  ...
  Future<List<Pet>> findAll() async {
    return _queryAdapter.queryList('SELECT * FROM Pet',
        mapper: (Map<String, Object?> row) =>
            Pet(row['name'] as String, Race.values[row['race'] as int]));
  }
  ...
  Future<int> deletePet(Pet pet) {
    return _petDeletionAdapter.deleteAndReturnChangedRows(pet);
  }
}
